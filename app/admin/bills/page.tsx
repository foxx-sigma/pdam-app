...
async function getBills(
  ...
  try {
    ...
    const url = new URL(`${process.env.NEXT_PUBLIC_BASE_API_URL}/bills`);
...
// Helper untuk cek status bill
function getBillStatus(bill: Bill): "unpaid" | "pending" | "paid" {
  if (bill.payments == null) return "unpaid";
  if (!bill.payments?.verified) return "pending";
  return "paid";
}

function StatusBadge({ status }: { status: "unpaid" | "pending" | "paid" }) {
  if (status === "paid") {
    return (
      <span
...
export default async function BillsPage({ searchParams }: Props) {
  ...
  if (status === "unpaid") {
    filteredBills = bills.filter((b) => getBillStatus(b) === "unpaid");
  } else if (status === "pending") {
    filteredBills = bills.filter((b) => getBillStatus(b) === "pending");
  } else if (status === "paid") {
    filteredBills = bills.filter((b) => getBillStatus(b) === "paid");
  }

  const totalAmount = filteredBills.reduce((sum, b) => sum + (b.amount ?? 0), 0);
  const paidCount = bills.filter((b) => getBillStatus(b) === "paid").length;
  const unpaidCount = bills.filter((b) => getBillStatus(b) === "unpaid").length;
  const pendingCount = bills.filter((b) => getBillStatus(b) === "pending").length;
  ...
  const stats = [
    ...
    {
      label: "Belum Lunas",
      value: unpaidCount + pendingCount,
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div className="rounded-2xl overflow-hidden" style={glassCard}>
        ...
        <div className="p-5 border-b flex flex-col md:flex-row md:items-center justify-between gap-4"
          ...
          <div>
            <h2 className="font-semibold" style={{ color: "#1e1b4b" }}>Data Tagihan</h2>
            <p className="text-xs mt-0.5" style={{ color: "rgba(107,114,128,0.8)" }}>
              {search
                ? `Hasil untuk "${search}" (${filteredBills.length} ditemukan)`
                : status !== "all"
                ? `Filter: ${status === "unpaid" ? "Belum Bayar" : status === "pending" ? "Menunggu Verifikasi" : "Lunas"} (${filteredBills.length} tagihan)`
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative w-56">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" fill="none" viewBox="0 0 20 20"
                style={{ color: "rgba(99,102,241,0.5)" }}>
                <path fillRule="evenodd"
                  d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z"
                  clipRule="evenodd" fill="currentColor" />
              </svg>
              <Search search={search} placeholder="Cari pelanggan..."
        ...
        {filteredBills.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="font-medium" style={{ color: "#1e1b4b" }}>
              {search ? `Tidak ada tagihan untuk "${search}"` : "Tidak ada data tagihan"}
            </p>
            ...
            {!search && <div className="mt-5"><AddBill /></div>}
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(99,102,241,0.1)", background: "rgba(99,102,241,0.03)" }}>
                    {["No", "Pelanggan", "No. Pelanggan", "Nominal Tagihan", "Pemakaian", "Status", "Tanggal Tagihan", "Aksi"].map((h) => (
                ...
                <tbody>
                  {filteredBills.map((bill, i) => {
                    ...
                    return (
                      <tr key={bill.id} className="transition-colors hover:bg-indigo-50/30"
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              <div className="font-medium" style={{ color: "#1e1b4b" }}>{bill.customer?.name ?? "-"}</div>
                              <div className="text-xs" style={{ color: "rgba(107,114,128,0.8)" }}>ID: {bill.customer_id}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3.5">
                          <code className="text-xs px-2 py-1 rounded-lg"
                            style={{ background: "rgba(99,102,241,0.07)", color: "rgba(99,102,241,0.9)" }}>
                        ...
                        <td className="px-4 py-3.5" style={{ color: "rgba(75,85,99,1)" }}>
                          {bill.usage_value != null ? `${bill.usage_value} m³` : "-"}
                        </td>
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-1">
                            <EditBill bill={bill} />
                            <DeleteBill billId={bill.id} customerName={bill.customer?.name} />
                          </div>
                        </td>
                      </tr>
                    );
